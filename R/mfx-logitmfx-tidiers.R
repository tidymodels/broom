...
#'
#' @param x A `logitmfx` object returned from [mfx::logitmfx()].
...
#' )
#' 
#' @details This function provides a tidy method for [mfx::logitmfx()], an
#'   enhanced implementation of binary logistic regression that calculates the 
...
#' )
#'
#' @details This augment method wraps [augment.glm()] for [mfx::logitmfx()] 
#'   objects.
#'
#' @export
#' @seealso [augment()], [augment.glm()], [mfx::logitmfx()]
#' @family mfx tidiers
#' @aliases mfx_tidiers
augment.logitmfx <- function(x, 
...
#' )
#'
#' @details This method wraps [glance.glm()] for [mfx::logitmfx()] objects.
#'   
#' @examples
#' library(mfx)
#' mfx_reg <- logitmfx(am ~ cyl + hp + wt, atmean = T, data = mtcars)
