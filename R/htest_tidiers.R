...
<<<<<<< HEAD
    ...
    if (length(ret$estimate) > 1) {
      ...
      if (grepl("Two Sample t-test", x$method)) {
        group_names <- sub("^mean (of|in group) ", "", names(x$estimate))
        ret <- c(group1 = group_names[1],
                 group2 = group_names[2],
                 estimate=ret$estimate1 - ret$estimate2, ret)
      }
    }
...
>>>>>>> 9ffd5d61c7ed74174191e3aab79f6083f08e7ed7
    }
  }
...
<<<<<<< HEAD
    unrowname(tibble::as_data_frame(ret))
=======
  }
