...
tidy.felm <- function(x,
  ...
  if (has_multi_response) {
    ret <-
      map_df(x$lhs, function(y)
        stats::coef(summary(x, lhs = y)) %>%
          fix_data_frame(nn) %>%
          mutate(response = y)) %>%
...
augment.felm <- function(x, data = model.frame(x), ...) {
  ...
  if (has_multi_response) {
    stop("Augment does not support linear models with multiple responses.",
      call. = FALSE
    )
  }
...
glance.felm <- function(x, ...) {
  ...
  if (has_multi_response) {
    stop("Glance does not support linear models with multiple responses.",
      call. = FALSE
    )
  }
  ret <- with(
