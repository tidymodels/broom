...
A
[`tibble::tibble()`](https://tibble.tidyverse.org/reference/tibble.html)
with one row for each original observation with addition columns:
...
See
[`joineRML::fitted.mjoint()`](https://rdrr.io/pkg/joineRML/man/fitted.mjoint.html)
and
[`joineRML::residuals.mjoint()`](https://rdrr.io/pkg/joineRML/man/residuals.mjoint.html)
for more information on the difference between population-level and
individual-level fitted values and residuals.

If fitting a joint model with a single longitudinal process, make sure
you are using a named `list` to define the formula for the fixed and
random effects of the longitudinal submodel.
