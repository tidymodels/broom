...
When `data` is not supplied `augment.factanal` returns one row for each
observation, with a factor score column added for each factor X,
(`.fsX`). This is because
[`stats::factanal()`](https://rdrr.io/r/stats/factanal.html), unlike
other stats methods like
[`stats::lm()`](https://rdrr.io/r/stats/lm.html), does not retain the
original data.

When `data` is supplied, `augment.factanal` returns one row for each
observation, with a factor score column added for each factor X,
(`.fsX`).
...
[`glance.factanal()`](https://broom.tidymodels.org/reference/glance.factanal.md),
[`tidy.factanal()`](https://broom.tidymodels.org/reference/tidy.factanal.md)
