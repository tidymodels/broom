...
* Added default argument `newdata = NULL` to `augment.prcomp` and `augment.rqs` (#543).
* Added default argyment `data = NULL` to `augment.factanal` and `augment.kmeans` (#543).
* Added default arguments for `type.predict` and `type.residuals` to `augment.betareg` that correspond to default arguments for `betareg` (#543).
