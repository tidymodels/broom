# test individual tidy.ZZZ methods

check_output <- function(o, exp.row=NULL, exp.col=NULL, exp.names=NULL) {
...
test_that("tidy.glm works", {
    ...
    td = tidy(glmfit)
    check_output(td, exp.row=2, exp.col=5)
    expect_equal(td$term, c("(Intercept)", "wt"))
    
    glmfit2 <- glm(cyl ~ wt + disp, mtcars, family="poisson")
    td2 = tidy(glmfit2)
    check_output(td2, exp.row=3, exp.col=5)
    expect_equal(td2$term, c("(Intercept)", "wt", "disp"))
})
...
test_that("tidy.anova and tidy.aov work", {
    ...
    td = tidy(anovafit)
    check_output(td, exp.row=3, exp.col=6)
...
test_that("tidy.nls works", {
    ...
    td = tidy(nlsfit)
    check_output(td, exp.row=3, exp.col=5)
