# test tidy, augment, glance from rma objects
...
test_that("tidy.rma works", {
    ...
    out <- structure(list(b = -0.714532348365101, se = 0.179781531780361, 
                          zval = -3.97444799412457, pval = 7.05426734852063e-05, ci.lb = -1.06689767574005, 
                          ci.ub = -0.362167020990149), .Names = c("b", "se", "zval", 
                                                                  "pval", "ci.lb", "ci.ub"), row.names = "intrcpt", class = "data.frame")
...
test_that("glance.rma works", {
    ...
    out <- structure(list(tau2 = 0.313243325980895, se.tau2 = 0.166427570950497, 
                          ...
                          H2 = 12.8557608031121, int.only = TRUE), .Names = c("tau2", 
                                                                              "se.tau2", "k", "p", "m", "QE", "QEp", "QM", "QMp", "I2", "H2", 
                                                                              "int.only"), row.names = c(NA, -1L), class = "data.frame")
    # dput(glance(fit))
